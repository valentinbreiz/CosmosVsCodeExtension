...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('cosmos.viewMemory', () => viewMemoryCommand(context))
    );
...
let memoryViewerPanel: vscode.WebviewPanel | undefined;

async function viewMemoryCommand(context: vscode.ExtensionContext) {
    // Check if we're in a debug session
    const debugSession = vscode.debug.activeDebugSession;
    if (!debugSession) {
        vscode.window.showWarningMessage('Start a debug session first to view memory');
        return;
    }
    ...
    const addressInput = await vscode.window.showInputBox({
        ...
        placeHolder: '0xffffffff80000000',
        value: '0xffffffff80000000'
    });

    if (!addressInput) return;

    // Create or show the memory viewer panel
    if (memoryViewerPanel) {
        memoryViewerPanel.reveal(vscode.ViewColumn.Two);
    } else {
        memoryViewerPanel = vscode.window.createWebviewPanel(
            'cosmosMemoryViewer',
            'Memory Viewer',
        ...
        memoryViewerPanel.onDidDispose(() => {
            memoryViewerPanel = undefined;
        });

        memoryViewerPanel.webview.onDidReceiveMessage(async message => {
            if (message.command === 'readMemory') {
                await readAndDisplayMemory(message.address, message.length);
    ...
    // Set initial content
    memoryViewerPanel.webview.html = getMemoryViewerHtml(addressInput);

    // Read initial memory
    await readAndDisplayMemory(addressInput, 256);
}

async function readAndDisplayMemory(addressStr: string, length: number) {
    const debugSession = vscode.debug.activeDebugSession;
    if (!debugSession || !memoryViewerPanel) return;

    try {
        // Use GDB command to read memory
        const address = addressStr.startsWith('0x') ? addressStr : `0x${addressStr}`;

        // Send command through debug console
        // Format: x/[count][format][size] address
        // x/64xb = 64 bytes in hex
        const response = await debugSession.customRequest('evaluate', {
            expression: `-exec x/${length}xb ${address}`,
            context: 'repl'
        });

        // Parse the GDB output and send to webview
        if (memoryViewerPanel) {
            memoryViewerPanel.webview.postMessage({
                command: 'updateMemory',
                address: address,
                data: response.result || 'Unable to read memory',
                raw: true
            });
        }
    } catch (error: any) {
...
function getMemoryViewerHtml(initialAddress: string): string {
    return `<!DOCTYPE html>
<html>
<head>
    <style>
...
<body>
    <div class="controls">
        <label>Address:</label>
        <input type="text" id="address" value="${initialAddress}" size="24">
        <label>Length:</label>
        <input type="number" id="length" value="256" min="16" max="4096" step="16">
        <button onclick="readMemory()">Read</button>
        <button onclick="refresh()">Refresh</button>
    </div>
    <div class="memory-display" id="memoryContent">
        Loading...
    </div>
    <script>
        ...
        function readMemory() {
            ...
            vscode.postMessage({ command: 'readMemory', address, length });
        }

        function refresh() {
            const address = document.getElementById('address').value;
            const length = parseInt(document.getElementById('length').value);
            vscode.postMessage({ command: 'refresh', address, length });
        }
        ...
        window.addEventListener('message', event => {
            ...
            if (message.command === 'updateMemory') {
                if (message.raw) {
                    // Display raw GDB output
                    content.innerHTML = '<pre>' + escapeHtml(message.data) + '</pre>';
                } else {
                    content.innerHTML = formatMemory(message.address, message.data);
                }
            } else if (message.command === 'error') {
                content.innerHTML = '<span class="error">Error: ' + escapeHtml(message.message) + '</span>';
            }
        });
        ...
        function formatMemory(baseAddr, hexData) {
            // Format as hex dump with ASCII
            let html = '';
            const bytes = hexData.match(/[0-9a-fA-F]{2}/g) || [];
            let addr = BigInt(baseAddr);

            for (let i = 0; i < bytes.length; i += 16) {
                const chunk = bytes.slice(i, i + 16);
                const addrStr = '0x' + addr.toString(16).padStart(16, '0');

                html += '<span class="address">' + addrStr + '</span>  ';

                // Hex bytes
                for (let j = 0; j < 16; j++) {
                    if (j < chunk.length) {
                        html += '<span class="hex-byte">' + chunk[j] + '</span> ';
                    } else {
                        html += '   ';
                    }
                    if (j === 7) html += ' ';
