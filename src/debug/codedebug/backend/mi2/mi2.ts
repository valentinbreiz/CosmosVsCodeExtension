...
export class MI2 extends EventEmitter implements IBackend {
	...
	load(cwd: string, target: string, procArgs: string, separateConsole: string, autorun: string[]): Thenable<any> {
		...
		return new Promise((resolve, reject) => {
			...
			if (process.platform === "win32") {
				...
				// Cosmos kernel debug never runs gdb with a separate tty — the
				// target is QEMU's gdbstub, not a hosted Linux process.
				promises.push(...autorun.map(value => { return this.sendUserInput(value); }));
				Promise.all(promises).then(() => {
					this.emit("debug-ready");
					resolve(undefined);
	...
	ssh(args: SSHArguments, cwd: string, target: string, procArgs: string, separateConsole: string, attach: boolean, autorun: string[]): Thenable<any> {
		// SSH transport was dropped from this vendored copy — Cosmos kernels
		// are debugged via a local gdb attached to QEMU's gdbstub, no remote
		// shell required.
		return Promise.reject(new Error("SSH debugging is not supported in cosmos-debug"));
	}
	...
	// SSH transport was stripped from this vendored copy; the fields remain as
	// `any` because a few helpers in this file still guard `isSSH` branches that
	// reference them. `isSSH` is never set to true under cosmos-debug, so these
	// stay undefined at runtime.
